...
import com.verygoodsecurity.vgscollect.view.internal.InputFieldFabric

abstract class BaseFieldView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr) {

    abstract val fieldType:FieldType

    private val inputField: BaseInputField by lazy {
        InputFieldFabric.getInputField(context, fieldType)
    }
...
//    internal val notifier = DependencyNotifier(inputField!!)
    ...
    internal fun getEditTextWrapper(): BaseInputField {
        return inputField
    }

    protected fun applyCardIconGravity(gravity:Int) {
        (inputField as? CardInputField)?.setCardPreviewIconGravity(gravity) //fixme
    }

    protected fun applyCardBrand(c: CustomCardBrand) {
        (inputField as? CardInputField)?.setCardBrand(c) //fixme
    }

    protected fun setNumberDivider(divider:String?) {
        (inputField as? CardInputField)?.setNumberDivider(divider) //fixme
    }
